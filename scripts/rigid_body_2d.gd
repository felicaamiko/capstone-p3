...
func _physics_process(delta):
	...
	if (Input.is_action_pressed("click")):#held down first frame
		if (firstframe and hovered and Global.selected == ""):
			Global.selected = str(name)
			print_debug(Global.selected)
			firstframe = false
			linear_damp = 0
		if (Global.selected == name):
			apply_force((tovector * sensitivity * jigglecheck)- linear_velocity, Vector2(0,0)) #if necessary, subtract linear_velocity to damp.
		if (not Global.selected == name):
			linear_damp = 1000
	else:
